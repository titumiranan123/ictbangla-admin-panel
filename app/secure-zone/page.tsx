...
const DashboardPage = () => {
  ...
  const calculateMetrics = (data: any) => {
    ...
    const monthsWithData = data?.enrollmentTrends?.filter((t: any) => t.paid > 0 || t.unpaid > 0);
    const avgConversion = monthsWithData?.length > 0 
      ? monthsWithData?.reduce((sum: number, t: any) => sum + (t.conversionRate || 0), 0) / monthsWithData?.length
      : 0;

    // Calculate total revenue
    const totalRevenue = data?.enrollmentTrends?.reduce((sum: number, t: any) => sum + t.revenue, 0);

    // Calculate month-over-month growth
    let momGrowth = 0;
    if (data?.enrollmentTrends?.length > 1) {
      const lastMonth = data?.enrollmentTrends[data?.enrollmentTrends.length - 1].revenue;
      const prevMonth = data?.enrollmentTrends[data?.enrollmentTrends.length - 2].revenue;
        ...
        callbacks: {
          label: function(context: any) {
            ...
            if (context.datasetIndex === 0) {
              const monthData = enhancedData?.enrollmentTrends[context.dataIndex];
              if (monthData) {
          ...
          footer: (tooltipItems: any[]) => {
            ...
            const monthData = enhancedData?.enrollmentTrends[tooltipItems[0].dataIndex];
            return [
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {Math.round(enhancedData?.summery?.totalUsers * 0.72).toLocaleString() || '0'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-600">New (30d)</span>
                  <span className="font-medium text-green-600">
                    +{enhancedData?.summery?.lastMonthUsers?.toLocaleString() ?? 0}
                  </span>
                </div>
              </div>
            </>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium text-green-600 flex items-center">
                    ...
                    {enhancedData?.summery?.totalPaidEnrol.toLocaleString() || '0'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-600">Completed</span>
                  <span className="font-medium text-indigo-600">
                    {enhancedData?.summery?.totalCompletedCourse.toLocaleString() || '0'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-600">LastWeekEnrolled</span>
                  <span className="font-medium">
                    {enhancedData?.summery?.lastWeekEnrolled?.toFixed(1) || '0'}%
                  </span>
                </div>
              </div>
            </>
          ...
          {isLoading ? (
            ...
            <>
              <p className="text-3xl font-bold text-green-600 mb-4">
                 Tk. {enhancedData?.summery?.totalRevenue?.toLocaleString() || '0'}
              </p>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-gray-600">This Month</span>
                  <span className="font-medium">
                  Tk. {enhancedData?.enrollmentTrends[enhancedData?.enrollmentTrends.length - 1]?.revenue.toLocaleString() || '0'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium text-green-600 flex items-center">
                    ...
                    {enhancedData?.summery?.monthOverMonthGrowth?.toFixed(1) || '0'}%
                  </span>
                </div>
              </div>
            </>
            ...
            <>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium text-green-600">
                    {enhancedData?.summery?.totalPublishedCourses.toLocaleString() || '0'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-600">Upcoming</span>
                  <span className="font-medium text-blue-600">
                    {enhancedData?.summery?.totalUpcomingCourses.toLocaleString() || '0'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="grid grid-cols-3 gap-4 mt-4 text-center">
              <div className="bg-gray-50 p-3 rounded-lg">
                ...
                <p className="font-bold text-green-600">
                  {enhancedData?.summery?.averageConversionRate?.toFixed(1) || '0'}%
                </p>
              </div>
              <div className="bg-gray-50 p-3 rounded-lg">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h3 className="text-md font-medium text-gray-700 mb-2 text-center">Instructors</h3>
                <div className="h-48">
                  <Doughnut 
              ...
              <div>
                <h3 className="text-md font-medium text-gray-700 mb-2 text-center">Courses</h3>
                <div className="h-48">
                  <Doughnut 
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {enhancedData?.topCourses.map((course: any) => (
                  <tr key={course.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-gray-900">{course.enrollments.toLocaleString()}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-green-600 font-medium"> Tk.{course.revenue.toLocaleString()}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
